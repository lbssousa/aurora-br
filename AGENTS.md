# aurora-br Copilot instructions

This document provides essential information for coding agents working with the aurora-br repository to minimize exploration time and avoid common build failures.

aurora-br is a Brazilian customization of [Aurora](https://github.com/ublue-os/aurora) (ublue-os/aurora). It shares the same architecture and build scripts, plus `build_files/br/` for Brazil-specific additions (Epson printers, BigLinux parental controls). Image vendor: `lbssousa`, registry: `ghcr.io/lbssousa/aurora-br`.

## Repository Overview

**aurora-br** is a Brazilian customization of Aurora, a cloud-native desktop OS that reimagines the Linux desktop experience. It's an immutable OS built on Fedora Linux using container technologies with atomic updates.
